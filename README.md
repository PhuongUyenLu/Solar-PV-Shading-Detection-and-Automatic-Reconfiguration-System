...
The DC bus bar serves as the main distribution point, collecting power from multiple PV arrays and supplying it to the inverter stage, where DC power is converted into AC power suitable for grid connection or load usage.

This baseline design reflects standard industry practices in large-scale PV systems and was used as a reference for implementing the shading detection and reconfiguration system in this project. It also provides a foundation for intelligent control strategies, such as shading detection and dynamic reconfiguration at the string or module level.

While this design primarily focuses on the DC side of the system, the complete architecture includes AC conversion and grid integration. After inversion, a transformer steps up the voltage to a higher level for efficient transmission and integration into the power grid.
